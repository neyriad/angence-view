...
export class PerformanceComponent implements OnInit {
  ...
  public getGraphic(): void {
    ...
    const observables: Observable<PerformanceDto>[] = this._selected.map((s: Consultant) => {
      return this.service.getBillingInRange(this.buildRequestParam(s)).pipe(catchError(error => of(error)));
    });
  ...
  public getPizzas(): void {
    ...
    const observables: Observable<PerformanceDto>[] = this._selected.map((s: Consultant) => {
      const filter = this.buildRequestParam(s);
      return this.service.getBillingForPercentage(filter).pipe(catchError(error => of(error)));
    });
